...
为了更好地兼顾潮汕各地发音，本辞典对传统的拼音方案进行了改进，制定了一套通用的标准音，各地可按各自的语言习惯转读，转读规则均为一对一映射（具体参见后文转读规则）。
辞典的这套“标准音”，我们不妨参照“老国音”的名字，称其为“**老潮音**”——它吸收了各地具有特色的发音，但本身并不真实反映今天某个特定区域的发音。

本辞典主要参考台罗方案，采用**白话字**（Péh-Uē-Jī, PUJ）拼音方案进行注音。
同时，辞典也提供了一套扩充的潮拼方案。在沿用现有潮拼方案的基础上（保留 -n 用于记录鼻化韵的习惯），补充对韵尾 -nd [n]、-d [t]，单元音 -er [ə] 复元音 -êu [eu] 等音素的记录，可参见[扩展潮拼及反切](./dp_fq.md)。
