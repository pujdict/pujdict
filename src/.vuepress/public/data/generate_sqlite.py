...
if __name__ == '__main__':
  ...
  c.execute('CREATE TABLE entries (entry_index int, char text, char_sim text, initial text, final text, tone int, cat text)')
  ...
  for entry in entries:
    c.execute('INSERT INTO entries VALUES (?, ?, ?, ?, ?, ?, ?)', (entry.entry_index, entry.char, entry.char_sim, entry.initial, entry.final, entry.tone, entry.cat))
  ...
  c.execute('INSERT INTO dbdesc VALUES (?, ?)', ('combinations', ','.join(sorted([f'{entry.initial}-{entry.final}-{entry.tone}' for entry in entries]))))
