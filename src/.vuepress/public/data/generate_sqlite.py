...
if __name__ == '__main__':
    ...
    c.execute(
        'CREATE TABLE entries (entry_index int, char text, char_sim text, initial text, final text, tone int, cat text, char_ref text, details text)')
    ...
    for entry in entries:
        c.execute('INSERT INTO entries VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
                  (entry.entry_index, entry.char, entry.char_sim, entry.initial, entry.final, entry.tone, entry.cat, entry.char_ref, entry.details))
