...
export default {
  ...
  methods: {
    queryPhrase(chars) {
      ...
      let result = [];
      let resultPhrases = [];
      // iterate through k, v pairs of charIndexing:
      for (const [key, phrases] of Object.entries(this.charIndexing)) {
    ...
    onInitFromDatabaseFinished() {
      ...
      for (const phrase of phrases) {
        ...
        (this.charIndexing[teochew] = this.charIndexing[teochew] || []).push(phrase);
        for (const cmn of cmn_list) {
          (this.cmnIndexing[cmn] = this.cmnIndexing[cmn] || []).push(phrase);
        }
      }
    },
  },
