...
// 已废弃。
const regexpWord = /^(?<initial>(p|ph|m|b|pf|pfh|mv(?=u)|bv(?=u)|f|t|th|n|l|k|kh|ng|g|h|ts|c|ch|tsh|chh|s|j|z|0)'?)?(?<final>(?<medial>(y|yi|i|u|iu)(?=[aeoiuvr]))?(?<nucleus>a|e|o|i|u|v|r|ng|n|m)(?<coda>(y|yi|i|u)?(m|n|ng|nn'?|p|t|k|h)*))?(?<tone>\d)?$/i;
// 保留 ir/ur/er 的版本。如果有 sirm 这样的组合，那么 ir 是一个整体。
const regexpWordOptional = /^(?<initial>(p|ph|m|b|pf|pfh|mv(?=u)|bv(?=u)|f|t|th|n|l|k|kh|ng|g|h|ts|c|ch|tsh|chh|s|j|z|0))?(?<final>(?<medial>(y|yi|i|u|iu)(?=[aeoiu]))?(?<nucleus>a|e|o|i|u|v|ur|ir|ṳ|or|er|o̤|ng|n|m)(?<coda>(y|yi|i|u)?(m|n|ng|nn'?|p|t|k|h)*))(?<tone>\d)?$/i;
