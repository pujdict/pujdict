...
// 已废弃。
const regexpWord = /^(?<initial>(p|ph|m|b|pf|pfh|phf|mv(?=u)|bv(?=u)|v|f|t|th|n|l|k|kh|ng|g|h|ts|c|ch|tsh|chh|s|j|z|0)'?)?(?<final>(?<medial>(y|yi|i|u|iu)(?=[aeoiuvr]))?(?<nucleus>a|e|o|i|u|ng|n|m)(?<coda>(y|yi|i|u)?(m|n|ng|nn'?|p|t|k|h)*))?(?<tone>\d)?$/i;
// 保留 ir/ur/er 的版本。如果有 sirm 这样的组合，那么 ir 是一个整体。
const regexpWordOptional = /^(?<initial>(p|ph|m|b|pf|pfh|phf|mv(?=u)|bv(?=u)|v|f|t|th|n|l|k|kh|ng|g|h|ts|c|ch|tsh|chh|s|j|z|0))?(?<final>(?<medial>(y|yi|i|u|iu)(?=[aeoiu]))?(?<nucleus>a|e|o|i|u|ur|ir|ṳ|or|er|o̤|ng|n|m)(?<coda>(y|yi|i|u)?(m|n|ng|nn'?|p|t|k|h)*))(?<tone>\d)?$/i;
const regexpWordDp = /(?<initial>(b|p|m|bh|bf|pf|mv|bhv|f|v|d|t|n|l|g|k|ng|gh|h|z|c|s|r|0))?(?<final>(?<medial>(i|u)(?=[aeoiu]))?(?<nucleus>a|e|ê|ê|ee|o|i|u|v|or|er|ng|m)(?<coda>(i|u)?(m|nd|ng|n'?|b|d|g|h)*))(?<tone>\d)?$/i;
